...
def test_binary(net, test_dataset, test_dataloader):
    ...
    prob_predictions = torch.zeros((len(test_dataset), 1), dtype=torch.int64)
    sample_counter = 0
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader, desc='Test'):
            ...
            for output in outputs_max.numpy():
                if sample_counter == 0:
                    csv_writer(thispath.parent, "final_predictions_binary.csv", "w", [output])
                else:
                    csv_writer(thispath.parent, "final_predictions_binary.csv", "a", [output])
                predictions[sample_counter] = output
                sample_counter += 1
    print(f"{sample_counter} predictions saved on {thispath.parent}/final_predictions_binary.csv")
    return predictions
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader, desc='Test'):
            ...
            for output in outputs_max:
                if sample_counter == 0:
                    csv_writer(thispath.parent, "final_predictions_multiclass.csv", "w", [output])
                else:
                    csv_writer(thispath.parent, "final_predictions_multiclass.csv", "a", [output])
                predictions[sample_counter] = output
                sample_counter += 1
...
def main(challenge_name, trained_net):
    ...
    if challenge_name == 'MulticlassClassification':
        test_multiclass(net, dataset_test, dataloader_test)
    elif challenge_name == 'BinaryClassification':
        test_binary(net, dataset_test, dataloader_test)
